...
func backOffTime(snap *Snapshot, parent, header *types.Header, val common.Address, chainConfig *chain.Config) uint64 {
	if snap.inturn(val) {
...
func (p *Parlia) epochLength(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) (uint64, error) {
	if header == nil {
		return params2.DefaultEpochLength, errUnknownBlock
	}
	if header.Number.Uint64() == 0 {
		return params2.DefaultEpochLength, nil
	}
	snap, err := p.snapshot(chain, header.Number.Uint64()-1, header.ParentHash, parents, false)
	if err != nil {
		return params2.DefaultEpochLength, err
	}
...
func (p *Parlia) BlockInterval(chain consensus.ChainHeaderReader, header *types.Header) (uint64, error) {
	if header == nil {
		return params2.DefaultBlockInterval, errUnknownBlock
	}
	if header.Number.Uint64() == 0 {
		return params2.DefaultBlockInterval, nil
	}
	snap, err := p.snapshot(chain, header.Number.Uint64()-1, header.ParentHash, nil, false)
	if err != nil {
		return params2.DefaultBlockInterval, err
	}
