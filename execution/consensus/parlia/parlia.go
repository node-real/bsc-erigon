...
const (
	...
	validatorBytesLength                   = length.Addr + bls2.PublicKeyLength
...
var (
	...
	// 100 native token
	maxSystemBalance = new(uint256.Int).Mul(uint256.NewInt(100), uint256.NewInt(common.Ether))
...
func ecrecover(header *types.Header, sigCache *lru.ARCCache[common.Hash, common.Address], chainId *big.Int) (common.Address, error) {
...
type Parlia struct {
	...
	recentSnaps *lru.ARCCache[common.Hash, *Snapshot]      // Snapshots for recent block to speed up
...
func New(
	...
	recentSnaps, err := lru.NewARC[common.Hash, *Snapshot](inMemorySnapshots)
	if err != nil {
		panic(err)
	}
	signatures, err := lru.NewARC[common.Hash, common.Address](inMemorySignatures)
	if err != nil {
		panic(err)
	}
...
func (p *Parlia) verifyVoteAttestation(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
	...
	votedAddrs := make([]bls2.PublicKey, 0, validatorsBitSet.Count())
	for index, val := range validators {
		if !validatorsBitSet.Test(uint(index)) {
			continue
		}

		voteAddr, err := bls2.NewPublicKeyFromBytes(snap.Validators[val].VoteAddress[:])
		if err != nil {
	...
	aggSig, err := bls2.NewSignatureFromBytes(attestation.AggSignature[:])
	if err != nil {
...
func (p *Parlia) verifyCascadingFields(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
	...
	if p.chainConfig.IsLorentz(header.Number.Uint64(), header.Time) {
		gasLimitBoundDivisor = params2.GasLimitBoundDivisor
	}
...
func (p *Parlia) snapshot(chain consensus.ChainHeaderReader, number uint64, hash common.Hash, parents []*types.Header, verify bool) (*Snapshot, error) {
	...
	for snap == nil {
		...
		if number == 0 {
			epochLength := params2.DefaultEpochLength
...
func (p *Parlia) distributeToSystem(val common.Address, ibs *state.IntraBlockState, header *types.Header,
	txs *types.Transactions, receipts *types.Receipts, systemTxs *types.Transactions,
...
func (p *Parlia) distributeToValidator(val common.Address, ibs *state.IntraBlockState, header *types.Header,
	txs *types.Transactions, receipts *types.Receipts, systemTxs *types.Transactions,
	usedGas *uint64, mining bool, systemTxCall consensus.SystemTxCall, curIndex, txIndex *int) (bool, error) {
	if *curIndex == *txIndex {
		bal, err := ibs.GetBalance(consensus.SystemAddress)
...
func (p *Parlia) applyTransaction(from common.Address, to common.Address, value *uint256.Int, data []byte,
	...
	if !bytes.Equal(actualHash.Bytes(), expectedHash.Bytes()) {
		return false, fmt.Errorf("expected system tx (hash %v, nonce %d, to %s, value %s, gas %d, gasPrice %s, data %s), actual tx (hash %v, nonce %d, to %s, value %s, gas %d, gasPrice %s, data %s)",
			...
			expectedTx.GetGasLimit(),
			expectedTx.GetTipCap().String(),
			hex.EncodeToString(expectedTx.GetData()),
			actualHash.String(),
			actualTx.GetNonce(),
			actualTx.GetTo().String(),
			actualTx.GetValue().String(),
			actualTx.GetGasLimit(),
			actualTx.GetTipCap().String(),
...
func (p *Parlia) ResetSnapshot(chain consensus.ChainHeaderReader, headers []*types.Header) error {
	...
	if checkpoint.Number.Uint64() == 0 {
		...
		validators, voteAddrs, err := parseValidators(checkpoint, p.chainConfig, params2.DefaultEpochLength)
		if err != nil {
			return err
		}
		// new snapshot
