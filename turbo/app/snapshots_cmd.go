...
func doBlkTxNum(cliCtx *cli.Context) error {
	...
	_, _, _, _, br, agg, clean, err := openSnaps(ctx, cfg, dirs, chainDB, nil, logger)
	if err != nil {
		return err
	}
	defer clean()
...
func openSnaps(ctx context.Context, cfg ethconfig.BlocksFreezing, dirs datadir.Dirs, chainDB kv.RwDB, bs services.BlobStorage, logger log.Logger) (
	...
	bscSnaps = freezeblocks.NewBscRoSnapshots(cfg, dirs.Snap, logger)
...
func openBlobStore(dirs datadir.Dirs, chainConfig *chain.Config, disableBlobPrune bool) services.BlobStorage {
	...
	var blobKept uint64
	blobKept = params2.MinBlocksForBlobRequests
	if disableBlobPrune {
