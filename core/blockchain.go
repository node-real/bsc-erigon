...
func FinalizeBlockExecution(
	...
	if isMining {
		...
		retRequests, err = engine.Finalize(cc, header, ibs, txs, uncles, receipts, withdrawals, chainReader, syscall, false, nil, 0, logger)
	}
	if err != nil {
		return nil, nil, err
	}
...
var systemContracts = map[common.Address]struct{}{
