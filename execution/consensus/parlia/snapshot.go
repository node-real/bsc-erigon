...
func newSnapshot(
	...
	snap := &Snapshot{
		...
		BlockInterval:    params2.DefaultBlockInterval,
...
func SnapshotFullKey(number uint64, hash common.Hash) []byte {
	return append(hexutil.EncodeTs(number), hash.Bytes()...)
}
...
func loadSnapshot(config *chain.ParliaConfig, sigCache *lru.ARCCache[common.Hash, common.Address], db kv.RwDB, num uint64, hash common.Hash) (*Snapshot, error) {
	...
	if snap.EpochLength == 0 { // no EpochLength field in old snapshots
		snap.EpochLength = params2.DefaultEpochLength
	}
	if snap.BlockInterval == 0 { // no BlockInterval field in old snapshots
		snap.BlockInterval = params2.DefaultBlockInterval
	}
...
func (s *Snapshot) store(db kv.RwDB) error {
	...
	return db.UpdateNosync(context.Background(), func(tx kv.RwTx) error {
		...
		if err = tx.Put(kv.ParliaSnapshot, []byte(lastSnapshot), hexutil.EncodeTs(s.Number)); err != nil {
			return err
		}
		return nil
	})
...
func (s *Snapshot) apply(headers []*types.Header, chain consensus.ChainHeaderReader, parents []*types.Header, chainConfig *chain.Config, recentSnaps *lru.ARCCache[common.Hash, *Snapshot], verified bool) (*Snapshot, error) {
	...
	for _, header := range headers {
		...
		*/
		...
		if snap.EpochLength == params2.DefaultEpochLength && chainConfig.IsLorentz(header.Number.Uint64(), header.Time) && nextBlockNumber%params2.LorentzEpochLength == 0 {
			...
			snap.EpochLength = params2.LorentzEpochLength
		}
		if snap.EpochLength == params2.LorentzEpochLength && chainConfig.IsMaxwell(header.Number.Uint64(), header.Time) && nextBlockNumber%params2.MaxwellEpochLength == 0 {
			snap.EpochLength = params2.MaxwellEpochLength
		}
...
func parseValidators(header *types.Header, chainConfig *chain.Config, epochLength uint64) ([]common.Address, []types.BLSPublicKey, error) {
	...
	for i := 0; i < n; i++ {
		cnsAddrs[i] = common.BytesToAddress(validatorsBytes[i*validatorBytesLength : i*validatorBytesLength+length.Addr])
