...
func (rw *HistoricalTraceWorker) RunTxTaskNoLock(txTask *state.TxTask) {
	...
	case txTask.Final:
		...
		_, err := rw.execArgs.Engine.Finalize(rw.execArgs.ChainConfig, types.CopyHeader(header), ibs, txTask.Txs, txTask.Uncles, txTask.BlockReceipts, txTask.Withdrawals, rw.chain, syscall, skipPostEvaluaion, nil, txTask.TxIndex, rw.logger)
		if err != nil {
			txTask.Error = err
		} else {
	...
	case txTask.SystemTxIndex > 0:
		...
		systemCall := func(ibs *state.IntraBlockState) ([]byte, bool, error) {
			...
			if rw.execArgs.ChainConfig.IsCancun(header.Time) {
		...
		_, err := rw.execArgs.Engine.Finalize(rw.execArgs.ChainConfig, types.CopyHeader(header), ibs, txTask.Txs, txTask.Uncles, txTask.BlockReceipts, txTask.Withdrawals, rw.chain, nil, false, systemCall, txTask.TxIndex, rw.logger)
		if err != nil {
			txTask.Error = err
		}
	default:
