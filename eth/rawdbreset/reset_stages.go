...
func ResetBlocks(tx kv.RwTx, db kv.RoDB, br services.FullBlockReader, bw *blockio.BlockWriter, dirs datadir.Dirs, chainReader consensus.ChainReader, engine consensus.Engine, logger log.Logger) error {
	// keep Genesis
	...
	if br.FrozenBlocks() > 0 {
		...
		if err := FillDBFromSnapshots("filling_db_from_snapshots", context.Background(), tx, dirs, br, chainReader, engine, logger); err != nil {
			return err
		}
...
func ResetPolygonSync(tx kv.RwTx, db kv.RoDB, br services.FullBlockReader, bw *blockio.BlockWriter, dirs datadir.Dirs, logger log.Logger) error {
	...
	if err := ResetBlocks(tx, db, br, bw, dirs, nil, nil, logger); err != nil {
		return err
	}
...
func FillDBFromSnapshots(logPrefix string, ctx context.Context, tx kv.RwTx, dirs datadir.Dirs, blockReader services.FullBlockReader, chainReader consensus.ChainReader, engine consensus.Engine, logger log.Logger) error {
	startTime := time.Now()
...
const (
	...
	*/
	pruneMarkerSafeThreshold = snaptype.Erigon2MergeLimit // 1.5x the merge limit
)
