...
func (sdc *SharedDomainsCommitmentContext) ResetCommitment() {
	sdc.updates.Reset()
}

func (sdc *SharedDomainsCommitmentContext) SaveCommitment(blockNum, txNum uint64, rootHash []byte) error {
	return sdc.encodeAndStoreCommitmentState(blockNum, txNum, rootHash)
}
...
func (sdc *SharedDomainsCommitmentContext) encodeAndStoreCommitmentState(blockNum, txNum uint64, rootHash []byte) error {
	...
	log.Trace("[commitment] store state", "block", blockNum, "txNum", txNum, "rootHash", hex.EncodeToString(rootHash))
