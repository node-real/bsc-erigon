...
func RemoteServices(ctx context.Context, cfg *httpcfg.HttpCfg, logger log.Logger, rootCancel context.CancelFunc) (
	...
	if cfg.WithDatadir {
		...
		onNewSnapshot = func() {
			wg.Go(func() (err error) {
				...
				if err := allBscSnapshots.OpenFolder(); err != nil {
		...
		if cc.Parlia != nil {
			...
			blobDb := kv2.New(dbcfg.BlobDB, log.New()).Path(blobDbPath).MustOpen()
	...
	if cfg.WithDatadir {
		if cc != nil && cc.Bor != nil {
			...
			bscKv, err = kv2.New(dbcfg.ConsensusDB, logger).Path(bscDbPath).Accede(true).Open(ctx)
			if err != nil {
				return nil, nil, nil, nil, nil, nil, nil, ff, nil, nil, err
			}
